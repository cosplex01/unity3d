...
Gdiplus::Image* g_pImage;

void OnGilPlusSetup() //생성자
{
	g_pImage = new Gdiplus::Image(L"C:/cs2019/cstudy/pub_res/basic_tile/basictiles.png");
}
void OnGilPlusFinish() // 소멸자
{
	delete g_pImage;
}
void OnGilPlusRender(double fDelta, Graphics* pGrp)
{
	//전체출력
	...
	//일부만 출력
	pGrp->DrawImage(g_pImage,
		Rect(250, 100, 16, 16), 16, 0, 16, 16,
		UnitPixel);
	pGrp->ScaleTransform(2, 2);
	pGrp->DrawImage(g_pImage,
		Rect(250, 130, 16, 16), 16, 0, 16, 16,
		UnitPixel);
	pGrp->ResetTransform();
}
                     ...
                     _In_ int       nCmdShow)
{
	...
	plusEngine::GDIPLUS_Loop(msg, Rect(0, 0, 320, 240),
		OnGilPlusSetup, NULL,OnGilPlusRender, OnGilPlusFinish);
