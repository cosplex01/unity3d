...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    {
	case WM_CREATE:
		MoveWindow(hWnd, 0, 0, 320, 340, true);
		break;
	case WM_KEYDOWN:
		g_keyTable[wParam] = 1;
	...
	case WM_KEYUP:
		...
		break;
	case WM_MOUSEMOVE:
		g_MouseInfo[0] = LOWORD(lParam); //x
		g_MouseInfo[1] = HIWORD(lParam); //y
		InvalidateRect(hWnd, NULL, true);
		break;
	case WM_LBUTTONDOWN:
		g_MouseInfo[2] = 1;
    ...
    case WM_COMMAND:
        {
			...
			case IDM_size640:
				{
					MoveWindow(hWnd, 330, 0, 640, 480, true);
					width = GetSystemMetrics(SM_CXSCREEN);
					height = GetSystemMetrics(SM_CYSCREEN);
				}
				break;
    ...
    case WM_PAINT:
        {
			...
			TCHAR szTemp[256];
			swprintf_s(szTemp, sizeof(szTemp) / sizeof(TCHAR), L"%8d%8d %8d%8d %8d%8d %8d %8d"
				,g_keyTable[VK_LEFT],
				g_keyTable[VK_RIGHT],
				g_MouseInfo[0],
				g_MouseInfo[1],
