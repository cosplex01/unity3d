...
void OnGidPlusRender(double fDelta, Graphics* pGrp)
{
	...
	irr::core::quaternion qt; //3차원을 이용하기 위함[사원수 계산용]
	qt.set(0,0,45); //3차원용 점을 지정함

	irr::core::vector3df vDir(30,0,0); //원점에서 30만큼 이동한 점
	
	_pen.SetColor(Color(200,0,200));

	qt.getMatrix().transformVect(vDir); //행렬을 거쳐서 2차원으로 변경하여 메트릭스에 처박음

	pGrp->DrawLine(&_pen,PointF(0,0),plusEngine::util::irr2Point(vDir));//결국 2차원의 위치로 이동함
	pGrp->ResetTransform();
}
