    ...
    case WM_COMMAND:
        {
			...
			case IDM_TEST:
			{
				int nResult = DialogBox(hInst, MAKEINTRESOURCE(IDD_DIALOG1), hWnd, Dialog1);
				if (nResult == IDOK) {
    ...
    case WM_PAINT:
        {
			...
			TCHAR szBuf[256];
			swprintf_s(szBuf,sizeof(szBuf)/sizeof(TCHAR),L"%d",g_count);
            EndPaint(hWnd, &ps);
        }
        break;
    case WM_DESTROY:
...
INT_PTR CALLBACK Dialog1(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDC_BUTTON1:
		{
			count++;
			TCHAR szBuf[256];
			swprintf_s(szBuf, sizeof(szBuf) / sizeof(TCHAR), L"count=%d", count);
