...
void OnRender(irr::f64 fDelta, Graphics* pGrp)
{
	...
	pGrp->DrawLine(&_pen,SCREEN_H / 2,0, SCREEN_W/2, SCREEN_H);

	pGrp->TranslateTransform(SCREEN_W / 2, SCREEN_H / 2); //가운데를 원점으로

	irr::core::vector2df pts[2] = { {-100,-100},{100,100} };
	
	irr::core::line2df line1 = irr::core::line2df(pts[0], pts[1]);

	pGrp->DrawLine(&_pen,pts[0].X,pts[0].Y,pts[1].X,pts[1].Y); //기하처리 클래스 변수를 사용함
	pGrp->DrawLine(&_pen, line1.start.X, line1.start.Y, line1.end.X, line1.end.Y); //기하처리 클래스 변수를 사용함1

	GraphicsState _gs = pGrp->Save(); //위치 상태 저장

	if (line1.getPointOrientation(testpt) > 0) { //testpt에 지정한 위치를 기준으로 오른손법칙 적용결과
		_pen.SetColor(Color::Aqua);
	}
	else
	{
		_pen.SetColor(Color::Beige);
	}

	pGrp->TranslateTransform(testpt.X,testpt.Y);
	pGrp->DrawEllipse(&_pen, Rect(-8,-8,16,16));

	pGrp->Restore(_gs); //위치 상태 로드
