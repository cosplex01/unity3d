...
int g_keyTable[1024];
S_GameObject g_GameObjs[32]; //32개의 접속만 허용
S_GameObject g_MyObjs;

void OnRender(double fDelta,Graphics* pGrp)
{
	if(fDelta >0){
		g_MyObjs.m_Xpos += g_keyTable[VK_LEFT] * fDelta * 25.0;
		g_MyObjs.m_Xpos -= g_keyTable[VK_RIGHT] * fDelta * 25.0;
		g_MyObjs.m_Ypos += g_keyTable[VK_UP] * fDelta * 25.0;
		g_MyObjs.m_Ypos -= g_keyTable[VK_DOWN] * fDelta * 25.0;

		if (g_hSocket != 0) {
			static double _accTick = 0;
			_accTick += fDelta;
			if (_accTick > 1.0) {
				_accTick = 0.0;//시간을 1초에...로 지정
				memset(&si_Serveraddr, 0, sizeof(si_Serveraddr));
				InetPton(AF_INET, L"192.168.0.15", &si_Serveraddr.sin_addr.S_un.S_addr); //L"는 전송할 주소"
				si_Serveraddr.sin_family = AF_INET;
				si_Serveraddr.sin_port = htons(3333);

				//&g_GameObj
				sendto(g_hSocket, (const char*)& g_MyObjs, sizeof(S_GameObject),
					0, (sockaddr*)& si_Serveraddr, sizeof(si_Serveraddr)); //서버에 데이터 전송
	...
	pGrp->Clear(Color::Aquamarine);

	pGrp->TranslateTransform(256, 256);//화면중간을 원점
	GraphicsState _gs;
                     ...
                     _In_ int       nCmdShow)
{
    ...
    // TODO: 여기에 코드를 입력합니다.
	for (int i = 0; i < 32; i++) {
		g_GameObjs[i].m_nStatus = 0;//not act
	}
    ...
    MSG msg;
	plusEngine::GDIPLUS_Loop(msg, Rect(0, 0, 512, 512), NULL, NULL, OnRender, NULL);
