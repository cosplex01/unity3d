...
//
//
void str_prn(HDC hDc, const wchar_t** ptr_ary, int count)
{
	for (int i = 0; i < count; i++) {
		TextOutW(hDc, 12, i * 20, ptr_ary[i], wcslen(ptr_ary[i]));
	}
}
    ...
    case WM_PAINT:
        {
			...
			str_prn(hdc, ptr_ary, _count);
			//TextOutW(hdc, 12, 20, L"그러게 말이야", wcslen(L"그러게 말이야"));
            EndPaint(hWnd, &ps);
        }
        break;
    case WM_DESTROY:
